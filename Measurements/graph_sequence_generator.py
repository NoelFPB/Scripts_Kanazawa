...
def create_simple_continuous_graphs(df, save_plots=True):
    ...
    df_sorted = df.sort_values('timestamp').reset_index(drop=True)
    df_sorted['timestamp_ms'] = df_sorted['timestamp'] * 1000
    
    # Set up the figure with 3 subplots sharing x-axis
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 10), sharex=True)
    
    # Graph 1: Input A vs Time - Simple continuous line
    ax1.plot(df_sorted['timestamp_ms'], df_sorted['A'], linewidth=2, color='blue', alpha=0.8)
    ax1.set_ylabel('Input A (V)', fontsize=24)
    # Example for one axis
    ax1.tick_params(axis='both', labelsize=14)  # change 18 to whatever size you want
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim(-0.5, 5.5)
    
    # Graph 2: Input B vs Time - Simple continuous line
    ax2.plot(df_sorted['timestamp_ms'], df_sorted['B'], linewidth=2, color='orange', alpha=0.8)
    ax2.set_ylabel('Input B (V)', fontsize=24)
    ax2.grid(True, alpha=0.3)
    ax2.set_ylim(-0.5, 5.5)
    ax2.tick_params(axis='both', labelsize=14)  # change 18 to whatever size you want
    
    # Graph 3: Output vs Time - Simple continuous line
    ax3.plot(df_sorted['timestamp_ms'], df_sorted['OUT'], linewidth=2, color='red', alpha=0.8)
    ax3.set_ylabel('Output (V)', fontsize=24)
    ax3.set_xlabel('Time (ms)', fontsize=24)   # update label here
    ax3.tick_params(axis='both', labelsize=14)  # change 18 to whatever size you want
    ...
    for i, row in df_sorted.iterrows():
        if row['state'] != prev_state:
            state_changes.append(row['timestamp_ms'])
            prev_state = row['state']
...
def main():
    ...
    df = load_data_from_file('RigolDS2.csv')
