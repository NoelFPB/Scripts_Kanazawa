...
class LogicGateOptimizer:
    ...
    def sample_most_uncertain(self, n_samples=5):
        """Sample configurations where the surrogate model is most uncertain."""
        if len(self.configs_evaluated) < 5:
            print("Not enough data for uncertainty sampling yet.")
            return

        print(f"Sampling {n_samples} most uncertain configurations...")
        
        # Generate random candidate configurations
        candidates = []
        for _ in range(200):  # You can tune this (more candidates = better uncertainty sampling)
        ...
        # Predict with all trees of the Random Forest
        X = np.array([self.config_to_vector(c) for c in candidates])
        preds = np.stack([tree.predict(X) for tree in self.model.estimators_], axis=0)  # (n_trees, n_samples)
        stds = np.std(preds, axis=0)  # Standard deviation across trees

        # Select top n_samples configurations with highest uncertainty
        top_indices = np.argsort(stds)[-n_samples:]
        ...
        for config in selected_configs:
            ...
            print(f"Uncertainty sample: Score = {score:.2f}")
    ...
    def optimize(self):
        """Run improved multi-stage optimization for logic gate."""
