...
INPUT_PINS = [37, 38]  # Input pins (A, B)
...
LOW_THRESHOLD = 2.3   # Outputs below this are considered LOW
OPTIMAL_LOW = 1.5     # Target for LOW output

HIGH_THRESHOLD = 4  # Outputs above this are considered HIGH  
OPTIMAL_HIGH = 4.5    # Target for HIGH output
...
class DecoderOptimizer:
    ...
    def evaluate_configuration(self, config):
        """Evaluate decoder configuration using a simplified weighted approach."""
        ...
        WEIGHTS = {
            'high_state': 0.40,  # HIGH output performance
            'low_state': 0.40,   # LOW output performance
        ...
        for test_config in TEST_CONFIGURATIONS:
            ...
            # Just a random function that can map the decoder behaviour
        ...
        final_score = sum(score * weight * 100 for score, weight in zip(component_scores.values(), WEIGHTS.values()))
        
        # Add success bonus
        success_count = sum(1 for r in results if r['correctly_decoded'])
        # success_bonus = (success_count / len(TEST_CONFIGURATIONS)) * 20
        ...
        if final_score > self.best_score:
            ...
            print(f"Components: HIGH={component_scores['high_state']:.2f}, LOW={component_scores['low_state']:.2f}, SEP={component_scores['separation']:.2f}")
