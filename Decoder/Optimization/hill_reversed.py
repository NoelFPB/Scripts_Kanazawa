...
def hill_climbing_optimization(ser, iterations=20):
    """Simple hill climbing optimization starting from highest numbered heater"""
    ...
    for iteration in range(iterations):
        ...
        # Try to improve each heater (in reverse order)
        for heater in modifiable_heaters:  # Already sorted in reverse order
            current_value = best_config[str(heater)]
            print(f"Testing heater {heater} (current value: {current_value}V)")
            ...
            for new_value in voltage_options:
                if new_value != current_value:
                    ...
                    if score > best_score:
                        ...
                        improved = True
                        print(f"  Improved! Score: {best_score} (changed heater {heater} to {new_value}V)")
                        print("  Restarting from highest heater...")
                        break
                    else:
                        print(f"  No improvement (score: {score})")
            
            if improved:
                break  # Go back to first (highest) heater
...
def main():
    try:
        ...
        print("Starting optimization (testing heaters in reverse order)...")
        print(f"Heater testing order: {modifiable_heaters}")
